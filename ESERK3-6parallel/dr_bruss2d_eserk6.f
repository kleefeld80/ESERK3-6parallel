...
c--------------------------------------------------------
      ...
      parameter (ns=500,neqn=ns*ns*2)
      double precision y(neqn), ans, xx, yy, rtol, atol, h, t, tend
      double precision truey(neqn), error
      external  fbrus
!
! measure CPU time
!
...
! ----- call of the subroutine SERK -----
      call ESERK6(neqn,t,tend,h,y,fbrus,atol,spcrad,iwork,idid)
      call itime(now)    ! now(1)=hour, (2)=minute, (3)=second
...
c------------------------------------------------------
      open(20,file='sol_exactbruss.out')
      read(20,*) truey
      error = 0.0d0
      k=1
      do j=1,ns
